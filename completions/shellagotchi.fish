...
complete -c shellagotchi -n "__fish_shellagotchi_needs_command" -f -a "hatch" -d 'Replace a dead pet with a brand-new newborn one. Only works while the current pet is actually dead (health reached 0); the old pet\'s final state is archived to the graveyard log before the new one is created'
...
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand hatch" -l species -d 'Species for the new pet: blob, cat, dragon, or ghost. Omit (or pass an unrecognized name) to pick one uniformly at random' -r
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand hatch" -s h -l help -d 'Print help'
...
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "feed" -d 'Report a command\'s exit status to the daemon (used by the shell hook)'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "daemon" -d 'Run the shellagotchi daemon (the process the shell hook and CLI subcommands talk to over a Unix socket)'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "prompt" -d 'Print a rendered prompt segment, reading ONLY the prompt cache file the daemon maintains (never the IPC socket). This makes it safe and fast enough to embed directly in a shell `PS1`'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "status" -d 'Show the pet\'s current status as a bordered ASCII card, fetched live from the daemon over IPC'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "show" -d 'Alias for `status` (the plan treats `show` and `status` as the same command)'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "watch" -d 'Launch a live, interactively-updating terminal UI showing the pet\'s sprite, mood, and stat gauges, polling the daemon in the background. Keybinds: q=quit, c=clean, p=pet, r=refresh'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "init" -d 'Print the shell integration snippet for `shell` to stdout, meant to be evaluated/sourced directly into an rc file, e.g. `eval "$(shellagotchi init bash)"` in `~/.bashrc`. Prints *exactly* the snippet with no extra logging, since the caller feeds stdout straight into `eval`/`source`'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "doctor" -d 'Run diagnostic checks (config, socket, daemon, rc-file hooks) and print a human-readable report. Exits non-zero if any check fails'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "install" -d 'Write the systemd user unit file and, if a systemd user session is available, enable + start the daemon via it'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "hatch" -d 'Replace a dead pet with a brand-new newborn one. Only works while the current pet is actually dead (health reached 0); the old pet\'s final state is archived to the graveyard log before the new one is created'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "gen-man" -d 'Generate the man page (troff, `clap_mangen`) to stdout. Hidden: this is a packaging-time helper (`shellagotchi gen-man > man/shellagotchi.1`), not a user-facing feature'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "gen-completions" -d 'Generate a shell completion script for `shell` (`clap_complete`) to stdout. Hidden: this is a packaging-time helper (`shellagotchi gen-completions bash > completions/shellagotchi.bash`), not a user-facing feature'
complete -c shellagotchi -n "__fish_shellagotchi_using_subcommand help; and not __fish_seen_subcommand_from feed daemon prompt status show watch init doctor install hatch gen-man gen-completions help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
